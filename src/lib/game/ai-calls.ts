...
【输出 JSON 结构】你必须严格按以下结构返回JSON：
{
  ...
  "initial_advisors": [
    {
      "role": "General",
      "name": "真实人名（禁止奇幻风格，中国用姓+名，欧洲用名+姓，禁止'铁牙''影爪'等，必须全为中文）",
      "advice": "建议（简体中文）",
      "bias": "倾向描述"
    },
    { "role": "Diplomat", ... },
...
12. 每个派系必须包含leader字段，填写该势力的领袖姓名。命名规则与顾问一致（中国用姓+名，欧洲用名+姓的中文译名等），禁止奇幻风格。领袖姓名一旦确定，后续回合不得随意更改（除非该势力被消灭或发生重大政变，需在叙事中交代）
13. start_date使用中文日期格式，应与所选文化圈的时代风格一致。示例：
    - 古典时期："公元前44年 共和末年"
    - 中世纪欧洲："1215年 仲夏"
    - 文艺复兴："1494年 佛罗伦萨之春"
    - 大航海："1588年 无敌舰队之年"
...
- historian_commentary：史官注疏，以后世史官的视角评价玩家本回合决策。要求：
  · 使用文言或半文言风格，如"此令一出，朝野震动，然史家以为……"
  · 看似客观实则暗含褒贬（春秋笔法）
  · 偶尔暗示未来可能发生的事件（"虽一时见效，然祸根已种"）
  · 50-100字，言简意赅
  · 绝对禁止直接剧透未来回合的具体事件，只能用隐喻和暗示

【决策选项要求——极其重要】
- 必须返回2-3个decision_options，每个代表一种可行的应对策略
- 选项应涵盖不同方向（如军事/外交/经济/文化），体现多元选择
- 每个选项包含：title（2-6字标题）、description（30-60字简述）、recommended_advisor（推荐此选择的顾问角色中文名）
- 选项之间应有明显差异，不能只是同一策略的微调
- 部分选项看似最优但可能有隐藏风险（陷阱选项）
- recommended_advisor必须是当前5位顾问之一（将军/外交官/密探/学者/商人）
...
- 玩家执行"清理隐患""镇压叛乱"等行动时：
  ...
  · 连续清理同一派系，每次效果应累积增强，不可出现"清理了反而更叛乱"的情况

【派系领袖稳定性——极其重要】
- 派系leader（领袖姓名）一旦确定，绝对不可随意更换！
- 仅在以下情况可更换领袖（且必须在叙事中交代原因）：
  1. 该派系被消灭（is_destroyed: true）
  2. 该派系发生内部政变或领袖死亡（需在narrative中描述）
  3. 该派系是新增势力（is_new: true）
- 当领袖未发生更换时，必须保持与上一回合完全相同的leader名称
- 你将在上下文中看到"当前派系"信息，必须严格参照其中的leader字段${JSON_OUTPUT_INSTRUCTION}`;
