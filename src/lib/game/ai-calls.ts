...
【输出 JSON 结构】你必须严格按以下结构返回JSON：
{
  ...
  "player_context": {
    ...
    "leader_title": "玩家头衔（如执政官、总督、大公、首相、摄政王、城邦领主等）",
...
严格规则：
1. 身份定义：必须明确玩家角色，给出具体的nation_name和leader_title。角色应与时代匹配，如：执政官、总督、大公、首相、摄政王、城邦领主、革命委员会主席、商会会长等
2. 多样性（最重要）：每次生成必须从不同的历史时期和文化圈中选取背景，严禁连续生成相同文化圈的剧本。可选文化圈包括但不限于：
   ...
   - 明治维新式：西化改革、传统与近代冲突
   - 冷战代理：意识形态对抗、代理人战争、核威慑
   - 中东/奥斯曼：苏丹、帕夏、宗教与世俗
   - 南亚/印度：土邦、殖民与独立、宗教冲突
   - 东南亚：海上贸易王国、殖民与独立
   - 非洲：王国兴衰、殖民与解放
   - 拉美：独立运动、考迪罗、革命
   - 东亚（非草原）：战国、幕府、王朝末期改革等
   注意：蒙古/草原/游牧汗国类背景已被过度使用，除非有极强的历史理由否则应避免
