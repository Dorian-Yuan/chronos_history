...
export function GameInput({
  ...
  const handleDecisionConfirm = useCallback((selectedTitles: string[]) => {
    const newText = selectedTitles.join("；");
    setInput((prev) => {
      ...
      <div className="flex items-center gap-2.5">
        {hasDecisionOptions && (
          <button
            ...
            disabled={disabled}
            className="flex h-10 w-10 shrink-0 items-center justify-center rounded-[var(--radius-md)] bg-accent-secondary text-text-inverse disabled:opacity-20 disabled:cursor-not-allowed active:scale-90 transition-all hover:bg-accent-secondary/90"
            aria-label="决策选择"
          >
            <Lightbulb size={16} />
          </button>
        ...
        <div className="flex-1 flex items-center rounded-[var(--radius-xl)] border border-border bg-bg-card shadow-sm pl-4 pr-1.5 py-1.5 focus-within:border-accent-primary/50 focus-within:shadow-glow transition-all">
          <textarea
            ref={textareaRef}
            value={input}
